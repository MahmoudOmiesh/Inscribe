import { memo } from "react";
import { cn } from "@/lib/utils";
import type { HeadingNode } from "../utils/types";
import { MarkRenderer } from "./mark-renderer";

export const Heading = memo(({ node }: { node: HeadingNode }) => {
  const className = cn(
    "whitespace-pre-wrap",
    node.text.length === 0 &&
      "after:pointer-events-none after:block after:content-['\\200b']",
  );

  switch (node.type) {
    case "heading-1":
      return (
        <h1 className={cn("text-4xl", className)} data-node-id={node.id}>
          <MarkRenderer text={node.text} marks={node.marks} />
        </h1>
      );
    case "heading-2":
      return (
        <h2 className={cn("text-3xl", className)} data-node-id={node.id}>
          <MarkRenderer text={node.text} marks={node.marks} />
        </h2>
      );
    case "heading-3":
      return (
        <h3 className={cn("text-2xl", className)} data-node-id={node.id}>
          <MarkRenderer text={node.text} marks={node.marks} />
        </h3>
      );
    case "heading-4":
      return (
        <h4 className={cn("text-xl", className)} data-node-id={node.id}>
          <MarkRenderer text={node.text} marks={node.marks} />
        </h4>
      );
  }
});

Heading.displayName = "Heading";
